...
func installCamoufox(executableName string) (string, error) {
	...
	if response.StatusCode != http.StatusOK {
		response.Body.Close()
		archive.Close()
		return "", fmt.Errorf("download Camoufox: HTTP %d", response.StatusCode)
	}
	...
	if copyErr != nil || closeErr != nil || archiveCloseErr != nil {
		return "", fmt.Errorf("save Camoufox: %w", firstError(copyErr, closeErr, archiveCloseErr))
	}
...
func extractCamoufoxArchive(archivePath string, destination string) error {
	...
	for _, entry := range archive.File {
		...
		if copyErr != nil || closeTargetErr != nil || closeSourceErr != nil {
			return fmt.Errorf("extract Camoufox %s: %w", entry.Name, firstError(copyErr, closeTargetErr, closeSourceErr))
		}
	}
	return nil
}
